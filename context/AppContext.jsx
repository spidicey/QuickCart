...
export const AppContextProvider = (props) => {
    ...
    const fetchProductData = async () => {
        try {
            ...
            if (result.success && Array.isArray(result.data)) {
                ...
                const transformedProducts = Object.values(groupedProducts).map(product => ({
                    ...product,
    ...
    const getCartDetails = () => {
        if (cartData && cartData.details) {
            return cartData.details.map(detail => ({
                productId: detail.variant.product_id,
                variantId: detail.variant.variant_id,
                sku: detail.variant.sku,
                quantity: detail.quantity,
                price: parseFloat(detail.variant.base_price),
                subPrice: parseFloat(detail.sub_price),
                image: detail.variant.image,
                size: detail.variant.attribute?.size,
                color: detail.variant.attribute?.color,
    ...
    const value = {
        currency, router, apiUrl,
        isSeller, setIsSeller,
        userData, setUserData, fetchUserData,
        products, fetchProductData, fetchProductById,
        lookbooks, fetchLookbooks, fetchLookbookById,
        cartItems, setCartItems, cartData,
        addToCart, updateCartQuantity, fetchCart,
        getCartCount, getCartAmount, getCartDetails,
        loading, error
    }
